...
main_clustering.py - This is the main pipeline to run everything

# Project Status

I think there are a few main problems to tackle:

1. How do we determine which pixels should be included for clustering? 
...
2. How should we cluster the pixel SEDs? 
    ...
    a. Normalize the pixels to remove the differences in brightness, then cluster using a standard distance measurement (euclidean metric). The "standard" distance would be the usual euclidean distance calcuation of sqrt((x_1-x_2)^2 + (y_1-y_2)^2 + (z_1-z_2)^2 + ...) for n dimensions. 
    ...
    It is unclear to me which will be more productive between a. and b. I think method b. offers a lot more flexibility , but will probably require more work to make sure it is working properly.

3. Once you have decided on the best way to prepare the pixel SEDs for clustering, what is the best algorithm to cluster them? 
